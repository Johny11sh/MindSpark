...
class _ChangeUsernameState extends State<ChangeUsername> {
  ...
  Future<Map<String, dynamic>?> changeUsernameData() async {
    try {
      // 1. Token validation
      final token = profileController.sharedPrefs.prefs.getString('token') ?? '';
      if (token.isEmpty) {
      ...
      if (response.statusCode == 200) {
        ...
        await profileController.sharedPrefs.prefs.setString('success', success!);

        if (responseBody['token'] != null) {
          await profileController.sharedPrefs.prefs.setString('token', responseBody['token']);
        }
                        ...
                        child: Container(
                          ...
                          child: MaterialButton(
                            onPressed: () async {
                              ...
                              isConnected = profileController.sharedPrefs.prefs.getBool(
                                'isConnected',
                              );
                              if (isConnected == true) {
                                if (formKey.currentState!.validate()) {
                                  changeUsernameData();
                                  success = profileController.sharedPrefs.prefs.getString(
                                    'success',
                                  );
