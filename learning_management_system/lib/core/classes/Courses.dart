...
class Courses extends StatefulWidget {
  ...
  String title;
   Courses({super.key, required this.courses, required this.CoursesImages,required this.title});
...
class _CoursesState extends State<Courses> {
  ...
  Future<void> _loadCachedData() async {
    try {
      // Load  courses data
      final CachedCourses = sharedPrefs.prefs.getString(
        'cached_courses',
      );
      if (CachedCourses != null) {
        ...
        widget.courses = List.from(cachedCourses);
      }

      // Load images for recommended courses
      await _loadCoursesImages();
    } catch (e) {
      debugPrint("Error loading cached data: $e");
    }
  }

  Future<void> _loadCoursesImages() async {
    for (var book in widget.courses) {
      final imageKey = 'book_image_${book['id']}';
      final cachedImage = sharedPrefs.prefs.getString(imageKey);
      if (cachedImage != null && mounted) {
        setState(() {
          widget.CoursesImages[book['id']] = base64Decode(cachedImage);
        });
      }
    }
  }
  ...
  Future<void> _cacheCourses() async {
    try {
      await sharedPrefs.prefs.setString(
        'cached_courses',
        jsonEncode(widget.courses),
      );
      cachedCourses = List.from(widget.courses);
    } catch (e) {
      debugPrint("Error caching courses: $e");
    }
  }

  Future<void> _cacheCourseImage(
    int bookId,
                              ...
                              child: ListView.builder(
                                ...
                                itemBuilder: (context, i) {
                                  try {
                                    ...
                                    Uint8List? imageBytes = widget.CoursesImages[courseId];
                                            ...
                                            Navigator.push(
                                              ...
                                              MaterialPageRoute(
                                                builder: (context) => BookDetails(
                                                  BookData: course,
                                                  bookImage: widget.CoursesImages[courseId],
                                                ),
                                              ),
                                            );
                                          },
                                    ...
                                    debugPrint("Error rendering course at index $i: $e");
                                    return Container(
